...
export default function DraftBoardPage() {
  ...
  // Live ESPN data — name → { adp, primaryPos, eligiblePos, espnRank }
  const [espnData, setEspnData] = useState<Record<string, { adp: number | null; primaryPos: string | null; eligiblePos: string[]; espnRank: number | null }>>({});
  ...
  const filtered = useMemo(() => {
    ...
    list = [...list].sort((a, b) => {
      switch (sortCol) {
        ...
        case "espn":    return dir * ((espnData[a.name]?.espnRank ?? 9999) - (espnData[b.name]?.espnRank ?? 9999));
